...
def fetch_and_export_orders():
    ...
    while True:
        # Fetch orders with cursor-based pagination (using `since_id`)
        if since_id:
            orders = shopify.Order.find(
                created_at_min=sixty_days_ago, status="any", limit=50, since_id=since_id)
        else:
            orders = shopify.Order.find(
                created_at_min=sixty_days_ago, status="any", limit=50)

        if not orders:
            break  # Exit the loop if no more orders are found

        for order in orders:
            # Only proceed if the order is not cancelled, paid, and fulfilled
            if order.cancelled_at is not None or (order.financial_status and order.financial_status.lower() != "paid") or (order.fulfillment_status and order.fulfillment_status.lower() != "fulfilled"):
                continue
        ...
        # Set `since_id` to the ID of the last order to get the next batch
